...
- **Campaign management** — Campaigns define a product description, target market, outreach parameters, and per-campaign LLM prompt overrides and pipeline condition overrides. Multiple campaigns can run in parallel. All configuration is managed via Django Admin.
- **Configurable LLM prompts & pipeline conditions** — All LLM system prompts (qualification, follow-up agent, fact extraction, memory reconciliation) are editable in Django Admin with global defaults and per-campaign overrides. Pipeline parameters (follow-up cooldown, qualification threshold, daily limits, etc.) follow the same global/campaign-level resolution.
- **Message rejection with feedback-driven regeneration** — When a follow-up draft is pending approval, operators can reject it with free-text feedback; the system immediately regenerates a new draft incorporating those instructions. Available via Django Admin action or CLI (`oo crm reject --feedback`).
