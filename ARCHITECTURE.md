...
- **Prompt templates** (at `linkedin/templates/prompts/`) — `qualify_lead.j2` (temp 0.7), `search_keywords.j2` (temp 0.9), `follow_up_agent.j2`. The follow_up template enforces: (a) Discovery-first strategy via Mom Test, (b) a hard rule to answer direct product questions before pivoting to discovery — prevents the agent from deflecting with a discovery question when the lead explicitly asks how the product works.
