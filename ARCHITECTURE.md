...
- **Prompt templates** (at `linkedin/templates/prompts/`) — `qualify_lead.j2` (temp 0.7), `search_keywords.j2` (temp 0.9), `follow_up_agent.j2`. The follow_up template enforces: (a) Discovery-first strategy via Mom Test, (b) a hard rule to answer direct product questions before pivoting to discovery, (c) greeting rules — greeting ("Hola, [name]!") is used only on the opening message (no prior "Me:" lines) or when re-engaging after `{{ reengagement_greeting_days }}` days of silence; in all other cases the LLM responds directly. The threshold is injected from `CAMPAIGN_CONFIG["reengagement_greeting_days"]` by `_render_system_prompt`, so it is tunable without touching the template.
