...
1. **`handle_connect`** — Unified via `ConnectStrategy` dataclass. Regular: `find_candidate()` from `pools.py`; freemium: `find_freemium_candidate()`. Unreachable detection after `MAX_CONNECT_ATTEMPTS` (6), marking the campaign deal FAILED without globally disqualifying the lead. No self-rescheduling — the planner owns timing.
